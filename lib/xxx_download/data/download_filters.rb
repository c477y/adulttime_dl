  ...
  module Data
    class DownloadFilters < Base
      ...
      def minimum_duration?(scene)
        ...
        scene_duration_t = Time.strptime(scene.duration, "%M:%S")
        minimum_duration_t = Time.strptime(minimum_duration, "%M:%S")
        ...
        raise XXXDownload::FatalError, "Invalid duration format(#{scene.duration}) for #{scene.title}. " \
          "Expected in MM:SS format"
      end

      def keyword_blocked?(scene)
        kw = skip_keywords.find do |kw|
