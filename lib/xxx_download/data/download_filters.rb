    ...
    class DownloadFilters < Base
      ...
      def skip?(scene)
        if scene.lesbian? && skip_lesbian?
          ...
          XXXDownload.logger.info "[SKIPPING NETWORK #{scene.network_name}] #{scene.file_name}"
          true
        elsif (performer = blocked_performers(scene))
          XXXDownload.logger.info "[SKIPPING PERFORMER #{performer}] #{scene.file_name}"
          true
        elsif (kw = blocked_keywords(scene))
          XXXDownload.logger.info "[SKIPPING KEYWORD #{kw}] #{scene.file_name}"
          true
