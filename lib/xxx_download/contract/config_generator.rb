...
module Contract
  class ConfigGenerator
    ...
    def generate
      ...
      XXXDownload.logger.debug(config.to_pretty_h)
      config
    end

    def hash_from_config_file
      @hash_from_config_file ||= XXXDownload::FileUtils.read_yaml(options[:config]).merge("site" => site)
    end
    ...
    def override_performers
      @override_performers ||= XXXDownload::FileUtils.read_yaml(options["performers"], nil, Array)
    end

    def override_movies
      @override_movies ||= XXXDownload::FileUtils.read_yaml(options["movies"], nil, Array)
    end

    def override_scenes
      @override_scenes ||= XXXDownload::FileUtils.read_yaml(options["scenes"], nil, Array)
    end
